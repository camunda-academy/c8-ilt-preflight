...
type Config struct {
	...
	// no longer testing the real SDK's own (nonexistent) proxy behavior — see
	// RUNBOOK's TypeScript section for the full tradeoff.
	TSProxySupport bool

	CohortID string // --cohort (opaque label only — never a customer name)
...
func Parse(argv []string) (*Config, error) {
	...
	tsProxySupport := fs.Bool("ts-proxy-support", false, "TypeScript only: route the SDK-snippet (tier 2) check through --proxy via a hand-written fetch override, instead of the default behavior of mirroring the real SDK exactly (which has zero proxy support and silently connects direct). Opt-in because it changes what's actually being tested — see RUNBOOK's TypeScript section (env: CAMUNDA_TS_PROXY_SUPPORT)")

	cohort := fs.String("cohort", "", "opaque cohort/engagement label for the result JSON — NEVER a customer name (see no-customer-data policy)")
