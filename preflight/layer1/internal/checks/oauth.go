...
// and reuses it — the RUNBOOK's OAuth rate-limit mitigation, ~1 req/sec/IP,
// depends on this.
	...
	if resp.StatusCode == http.StatusTooManyRequests {
		return "", model.Stage{
			...
			Detail: "OAuth host rate-limited this request (~1 token/sec/IP shared across all clients on this network). " +
				"Stagger participant runs or rely on network mode for the bulk check (see RUNBOOK OAuth rate-limit caveat).",
			ElapsedMs: elapsed,
		}
	}
