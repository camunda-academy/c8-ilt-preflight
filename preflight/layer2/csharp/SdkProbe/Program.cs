...
// JVM system properties, not env vars). See
// the training documentation's C# section.
//
...
async Task<int> RunAsync(string[] a)
{
    ...
    using (client)
    {
        ...
        catch (CamundaAuthException cae)
        {
            ...
            // secret) -- verified with deliberately fake credentials.
        ...
        else
        {
            ...
            catch (CamundaAuthException cae)
            {
                ...
                // OAuth-token-fetch-failure class, verified with fake
                // credentials.
