...
# be generated per environment; see RUNBOOK for how to produce one.
...
def install_sdk():
    ...
    if lock:
        ...
        eprint("[python sdk-probe] instead, or drop a hash-locked %s next to this probe (see RUNBOOK)." % LOCK_FILENAME)
...
def trust_store_label():
    ...
    # RUNBOOK.md for trainers, not repeated here every run.
    import os
...
def run_checks(sdk_mod, sdk_errors):
    ...
    # hit the identical opaque 404 (see RUNBOOK).
    ...
    # auto-infer OAUTH and authenticate a "credential-free" run (verified live:
    ...
    # in the SDK, worth reporting upstream — see RUNBOOK). Only passing
