 ...
 *
 * Auth-strategy auto-upgrade gap, VERIFIED (mirrors Python's known gap
 ...
 * -- verified that an explicit value (via env OR this override) fully
...
function installSdk() {
  ...
  try {
    execFileSync(npmCmd, ['ci', '--no-audit', '--no-fund'], {
      ...
      // EINVAL (verified in this environment) -- npm.cmd is a batch
 ...
 * dispatcher for exactly this use case -- confirmed it: (a) correctly
...
async function runChecks(sdkModule) {
  ...
  // (nonexistent) proxy handling -- see the TypeScript notes for the
  // full tradeoff.
    ...
    fragments.push(
      fragment(
        ...
        'A proxy is configured (--proxy/HTTPS_PROXY) but --ts-proxy-support/CAMUNDA_TS_PROXY_SUPPORT is not set. The ' +
          ...
          '--ts-proxy-support to route this check through the proxy instead (see the training documentation for what that does and does ' +
          "not prove)."
      )
    );
  }
  ...
  try {
    const client = createCamundaClient({
      config,
      // NullLogger equivalent -- REQUIRED, not optional. Verified: at
      ...
      // also verified: unlike Python (where CAMUNDA_SDK_LOG_LEVEL=silent
...
async function main() {
  ...
  // transitive dependency ever prints a banner at import time. Verified
